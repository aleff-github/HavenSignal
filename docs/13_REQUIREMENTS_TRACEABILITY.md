...
| SEC-DEL-001..006 | CRITICAL/HIGH | 03 Lifecycle; 04 Cryptographic Model; 32 Retention/Deletion | 14/32: inert operator order plus key destruction/restore, deletion state/race, blob retry tests | Exact OPEN-only operator order and cleanup timing modeled non-authorizing with unavailable executors; legal/review, persistence, audit, MFA, alert, storage, and Key Service gates OPEN CRITICAL |
