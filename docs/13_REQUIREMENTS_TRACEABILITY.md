...
| SEC-RECOVERY-001..004 | CRITICAL | 05 Recovery Response; 20 Submission Protocol; 21 Recovery Credential Construction | 14: recovery enumeration/secret-handling tests plus inert attempt-credential, duplicate/retry no-redisplay, credential-shape, recovery failure, recovery key lifecycle, recovery verification, and one-time credential-response source-conformance tests | Owner-approved attempt-credential boundaries, duplicate/retry no-redisplay outcomes, recovery credential shapes, recovery failure behavior, Recovery Verifier key lifecycle and verification metadata, and one-time response/lost-response policy have content-free Stage A descriptors and exact source guards; generation, verifier execution, lookup, persistence, endpoint, independent cryptographic review, and dependent gates remain OPEN |
| SEC-RECOVERY-005 | CRITICAL | 05 Recovery Response; 12 Open Decisions; 21 Recovery Credential Construction | Owner decision, inert recovery verification source-conformance tests, and independent cryptographic design review | Owner-approved verifier semantics are represented inertly; independent review required before implementation |
