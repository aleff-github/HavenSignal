"""Static abuse tests for the inert audit-v1 descriptor module."""
...
TARGET = BASE_DIR / AUDIT_DESCRIPTOR_PATH


class AuditDescriptorSourcePolicyTests(SimpleTestCase):
    @classmethod
    ...
    def analyze(self, source: str):
        return analyze_audit_descriptor_source(source=source)

    def test_current_source_matches_the_exact_inert_profile(self) -> None:
        self.assertEqual(
            scan_audit_descriptor_source(path=TARGET, relative_to=BASE_DIR),
            (),
        )

    def test_import_class_registry_and_validator_changes_are_rejected(self) -> None:
        mutations = (
            self.source.replace(
    ...
    def test_success_side_effect_and_dynamic_behavior_are_rejected(self) -> None:
        mutations = (
            self.source.replace(
    ...
    def test_target_parse_and_path_failures_are_controlled(self) -> None:
        wrong_target = analyze_audit_descriptor_source(
            source=self.source,
        ...
        for path, root in (
            ...
            with self.subTest(path=path):
                violations = scan_audit_descriptor_source(
                    path=path,
                    relative_to=root,
                )
