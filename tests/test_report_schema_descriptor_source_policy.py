"""Static abuse tests for inert original-report schema descriptors."""
...
TARGET = BASE_DIR / REPORT_SCHEMA_DESCRIPTOR_PATH


class ReportSchemaDescriptorSourcePolicyTests(SimpleTestCase):
    @classmethod
    ...
    def test_current_source_matches_the_exact_inert_profile(self) -> None:
        self.assertEqual(
            scan_repository_report_schema_descriptor(BASE_DIR),
            (),
        )
    ...
    def test_import_schema_and_field_changes_are_rejected(self) -> None:
        ...
        for source in mutations:
            with self.subTest(source=source[:80]):
                violations = analyze_report_schema_descriptor_source(source=source)
    ...
    def test_target_parse_and_path_failures_are_controlled(self) -> None:
        wrong_target = analyze_report_schema_descriptor_source(
            source=self.source,
        ...
        parse_failure = analyze_report_schema_descriptor_source(
            source="def broken(\n"
        )
        ...
        with TemporaryDirectory() as temporary_directory:
            violations = scan_repository_report_schema_descriptor(
        ...
        violations = scan_report_schema_descriptor_source(
            path=TARGET,
    ...
    def test_source_is_never_executed_or_echoed(self) -> None:
        ...
        violations = analyze_report_schema_descriptor_source(
            source=self.source + f"\nraise RuntimeError('{sentinel}')\n"
        )
        self.assertEqual(len(violations), 1)
        self.assertNotIn(sentinel, repr(violations))

        parse_violations = analyze_report_schema_descriptor_source(
